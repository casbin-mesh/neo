...
func BenchmarkSingleParse(bench *testing.B) {
	for i := 0; i < bench.N; i++ {
		s := NewLexer(strings.NewReader("1"))
		yyParse(s)
	}
}
...
func BenchmarkEvaluationSingle(bench *testing.B) {
	s := NewLexer(strings.NewReader("1"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	bench.ResetTimer()
	for i := 0; i < bench.N; i++ {
		expression.Evaluate(nil)
	}
...
func BenchmarkEvaluationNumericLiteral(bench *testing.B) {
	s := NewLexer(strings.NewReader("(2) > (1)"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	bench.ResetTimer()
	for i := 0; i < bench.N; i++ {
...
func BenchmarkEvaluationLiteralModifiers(bench *testing.B) {
	...
	s := NewLexer(strings.NewReader("(2) + (2) == (4)"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	bench.ResetTimer()
	for i := 0; i < bench.N; i++ {
...
func BenchmarkEvaluationParameters(bench *testing.B) {
	...
	s := NewLexer(strings.NewReader("requests_made > requests_succeeded"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	ctx := ast.NewContext().(*ast.Context)
	ctx.GetParameters().(ast.ParameterSet).AddParameter("requests_made", ast.Primitive{Typ: ast.INT, Value: 99})
	ctx.GetParameters().(ast.ParameterSet).AddParameter("requests_succeeded", ast.Primitive{Typ: ast.INT, Value: 90})
...
func BenchmarkEvaluationParametersModifiers(bench *testing.B) {
	...
	s := NewLexer(strings.NewReader("(requests_made * requests_succeeded / 100) >= 90"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	ctx := ast.NewContext().(*ast.Context)
	ctx.GetParameters().(ast.ParameterSet).AddParameter("requests_made", ast.Primitive{Typ: ast.INT, Value: 99})
...
type mockFunc struct {
	fn func(args ...*ast.Primitive) (*ast.Primitive, error)
}

func (f *mockFunc) Eval(args ...*ast.Primitive) (*ast.Primitive, error) {
	return f.fn(args...)
}

func BenchmarkEvaluationFunction(bench *testing.B) {

	s := NewLexer(strings.NewReader("(get_requests_made() * requests_succeeded / 100) >= 90"))
	yyParse(s)
	expression := s.parseResult.(ast.Evaluable)
	ctx := ast.NewContext().(*ast.Context)
	ctx.GetParameters().(ast.ParameterSet).AddParameter("requests_succeeded", ast.Primitive{Typ: ast.INT, Value: 90})
	ret := &ast.Primitive{Typ: INT, Value: 99}
	fn1 := mockFunc{
		fn: func(args ...*ast.Primitive) (*ast.Primitive, error) {
			return ret, nil
		},
	}
	ctx.GetFunctions().(ast.FunctionSet).AddFunction("get_requests_made", &fn1)
